...
from analysis import web_build_packages as package
from analysis import web_filter_companies as filter_companies
...
def create_app():
    ...
    class ExpertForm(FlaskForm):
        ...
        average_high_threshold = IntegerField("Threshold Hoher Durchschnitt",default=10)
        average_high_strikes = IntegerField("Strikes",default=1)
        average_medium_threshold = IntegerField("Threshold Mittlerer Durchschnitt",default=5)
        average_medium_strikes = IntegerField("Strikes",default=1)
        average_low_threshold = IntegerField("Threshold Niedriger Durchschnitt",default=2)
        ...
        median_medium_threshold = IntegerField("Threshold Durschnitts Median",default=5)
        median_medium_strikes = IntegerField("Strikes",default=1)
        median_low_threshold = IntegerField("Threshold Niedriger Median",default=1)
        median_low_strikes = IntegerField("Strikes",default=1)
        bad_trades_threshold = IntegerField("Threshold Negativer Trades",default=10)
        bad_trades_strikes = IntegerField("Strikes",default=2)
        bad_trades2_threshold = IntegerField("Threshold2 Negativer Trades",default=20)
        bad_trades2_strikes = IntegerField("Strikes",default=4)
        severe_trades_threshold = IntegerField("Threshold Trades unter -10%",default=10)
        severe_trades_strikes = IntegerField("Strikes",default=4)
        great_trades_threshold = IntegerField("Threshold Trades über 10%",default=6)
        great_trades_strikes = IntegerField("Strikes",default=1)
        averages_multiplier = IntegerField("Multiplier für Durschnitt",default=100)
        medians_multiplier = IntegerField("Multiplier für Median",default=100)
        bad_trades_multiplier = IntegerField("Multiplier für Negative Trades",default=100)
        severe_trades_multiplier = IntegerField("Multiplier für Trades unter -10%",default=100)
        great_trades_multiplier = IntegerField("Multiplier für Trades über 10%",default=100)
        submit = SubmitField()
    ...
    def landing():
        if request.method == "POST":
            ...
            high_hists, medium_hists, low_hists, package_objects, breakdowns_per_year = package.web_call(timeframe_buy, timeframe_sell, start_year, end_year)
            total_values_high = []
            total_values_medium = []
            total_values_low = []
    ...
    def full_controll():
        ...
        if request.method == "POST":
            form_data = form.data
            high_hists, medium_hists, low_hists, package_objects, breakdowns_per_year = filter_companies.webcall(form_data)
            total_values_high = []
            total_values_medium = []
            total_values_low = []
