...
def package_loop_datetime(ranking, counter, last_date=None):
    # 0 start_date, 1 end_date, 2 points, 3 id, 4 name
    if last_date != None:
        last_date = date(int(last_date[:4]), int(
            last_date[5:7]), int(last_date[-2:]))
        for x in range(len(ranking)):
            if x >= counter:
                if ranking[x][0] >= last_date:
                    package = [ranking[x]]
...
def get_firms_from_db(raw_data, raw_data2, timeframe, year, amount_high, amount_medium, amount_low, tax_credit_high, tax_credit_low, last_date=None):
    ...
    amount_high, amount_medium, amount_low, last_date, high_hist, medium_hist, low_hist, package_object, backtest_breackdowns, tax_credit_high, tax_credit_low = backtest.get_packages(
        package, timeframe, year, amount_high, amount_medium, amount_low, tax_credit_high, tax_credit_low)
...
def web_call(timeframe_buy, timeframe_sell, start_year, end_year):
    ...
    years = total_years[total_years.index(
        int(start_year)):total_years.index(int(end_year))+1]
    ...
    for year in years:
        db_cursor.execute(
            f"SELECT * FROM '{year-1}' WHERE Timeframe = '{timeframe}'")
        raw_data = db_cursor.fetchall()
        db_cursor.execute(
            f"SELECT * FROM '{year}' WHERE Timeframe = '{timeframe}'")
        raw_data2 = db_cursor.fetchall()
        if year == years[0]:
            amount_high, amount_medium, amount_low, last_date, high_hist, medium_hist, low_hist, package_object, backtest_breackdowns, tax_credit_high, tax_credit_low = get_firms_from_db(
                raw_data, raw_data2, timeframe, year, amount_highs[-1], amount_mediums[-1], amount_lows[-1], tax_credit_high, tax_credit_low)
        else:
            amount_high, amount_medium, amount_low, last_date, high_hist, medium_hist, low_hist, package_object, backtest_breackdowns, tax_credit_high, tax_credit_low = get_firms_from_db(
                raw_data, raw_data2, timeframe, year, amount_highs[-1], amount_mediums[-1], amount_lows[-1], tax_credit_high, tax_credit_low, last_date)
