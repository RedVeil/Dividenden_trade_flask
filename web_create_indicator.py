    ...
    for key in companies.keys():
        for n in range(len(averages_rank)):
            if key in averages_rank[n]:
                companies[key].points += n * (form_data["averages_multiplier"]/100)
            if key in medians_rank[n]: 
                companies[key].points += n * (form_data["medians_multiplier"]/100)
            if key in bad_trades_rank[n]:
                companies[key].points += n * (form_data["bad_trades_multiplier"]/100)
            if key in severe_trades_rank[n]:
                companies[key].points += n * (form_data["severe_trades_multiplier"]/100)
            if key in great_trades_rank[n]:
                companies[key].points += n * (form_data["great_trades_multiplier"]/100)
...
def get_accepted_companies(accepted_companies, timeframe, form_data):
    ...
    years = total_years[total_years.index(int(form_data["start_year"])-1):total_years.index(int(form_data["end_year"]))+1]
    companies_last_year = {}
    high_hists = {}
    medium_hists = {}
    low_hists = {}
    ...
    for year in years:
        ...
        if year == years[0]:
            ...
            amount_high, amount_medium, amount_low, last_date, high_hist, medium_hist, low_hist, package_object, backtest_breackdowns, tax_credit_high,tax_credit_low = packages.expert_get_firms_from_db(companies_last_year, companies, timeframe, year, amount_highs[-1], amount_mediums[-1], amount_lows[-1], tax_credit_high, tax_credit_low)
            companies_last_year = companies
            ...
            amount_high, amount_medium, amount_low, last_date, high_hist, medium_hist, low_hist, package_object, backtest_breackdowns, tax_credit_high,tax_credit_low = packages.expert_get_firms_from_db(companies_last_year, companies, timeframe, year, amount_highs[-1], amount_mediums[-1], amount_lows[-1], tax_credit_high, tax_credit_low, last_date)
            companies_last_year = companies
            high_hists[year] = high_hist
            medium_hists[year] = medium_hist
            low_hists[year] = low_hist
